  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-6 space-y-6">
      ...
      <div className="flex gap-2">
        ...
        />
        <button className="bg-green-900 text-white px-4 py-2 rounded-md text-sm hover:bg-green-800">
          Apply coupon
        </button>
      </div>
      ...
      <div className="space-y-4 text-sm">
        ...
        ].map(({ label, type, placeholder, isHalf }, i, arr) =>
          isHalf ? null : (
            <div key={label}>
              <label>{label}</label>
              <input
      ...
      {/* Pay Button */}
      <button className="w-full bg-green-900 text-white py-3 rounded-xl font-medium text-lg hover:bg-green-800">
        Pay ${total.toFixed(2)}
      </button>

      {/* Cashback Card */}
      <div className="bg-[#fdf4eb] text-sm p-4 rounded-md">
        <span className="font-semibold text-green-800">Earn 5% cash back</span>{" "}
        on Shopcart <br />
